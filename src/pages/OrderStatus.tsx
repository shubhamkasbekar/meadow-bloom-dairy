    ...
    if (user) {
      if (orders.some(order => order.userId === user.id)) {
        ...
        const mockOrder: Order = {
          ...
          userId: user.id,
          items: [
            { product: orders[0].items[0].product, quantity: 1 },
          ],
        ...
        setUserOrders([mockOrder, ...orders.filter(order => order.status !== "placed")]);
      }
    }
  }, [user]);
                ...
                <CardContent className="p-6">
                  ...
                  <div className="mb-8">
                    ...
                    <div className="flex justify-between mt-2">
                      <div className={`text-xs ${order.status === 'placed' || order.status === 'processing' || order.status === 'out-for-delivery' || order.status === 'delivered' ? 'text-blue-600' : 'text-gray-400'}`}>
                        Order Placed
                      </div>
                      <div className={`text-xs ${order.status === 'processing' || order.status === 'out-for-delivery' || order.status === 'delivered' ? 'text-yellow-600' : 'text-gray-400'}`}>
                        Processing
                      </div>
                      <div className={`text-xs ${order.status === 'out-for-delivery' || order.status === 'delivered' ? 'text-violet-600' : 'text-gray-400'}`}>
                        Out for Delivery
                      </div>
                      <div className={`text-xs ${order.status === 'delivered' ? 'text-green-600' : 'text-gray-400'}`}>
                        Delivered
                      </div>
                    </div>
                  </div>
